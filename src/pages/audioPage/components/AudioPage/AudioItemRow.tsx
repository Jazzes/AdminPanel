...
import "./AudioItemRow.scss"
import {Audio, Fimbo} from "../../../../models/Models";
import {Link} from "react-router-dom";
import ImgPreview from "../../../../components/PhotoPreview/ImgPreview";

interface IAudioItemRow {
    audio: Audio
    index: number
    filesHost: string
    fimbos: Fimbo[]
...
const AudioItemRow : FC<IAudioItemRow> = ({audio, index, filesHost, fimbos}) => {

    const noImg = useRef(false)
    const [imgPreview, setImgPreview] = useState(false)

    const fimboName = (id: number) => {
        for (let i = 0; i < fimbos.length; ++i) {
            if (id === fimbos[i].id) {
                return fimbos[i].name
            }
        }
    }

    const fimboPath = (id: number) => {
        for (let i = 0; i < fimbos.length; ++i) {
            if (id === fimbos[i].id) {
                return fimbos[i].path
            }
        }
    }

    const imgURL = `${filesHost}/files/audio/${fimboPath(audio.fimbo_id)}/${audio.fimbo_size}/${audio.type}/${audio.path}/${audio.img}`
    ...
    return (
        <>
            <div className={index % 2 === 1 ? "AudioItemRow__row" : "AudioItemRow__row AudioItemRow__row__color"}
                 style={audio.visible ? {} : {background: "none", color: "#606060"}}>
                <div className="AudioItemRow__row__visible">
                    {audio.visible ?
                ...
                <div className="AudioItemRow__row__type">
                    {audio.type}
                </div>

                <div className="AudioItemRow__row__preview">
                    <img className="AudioItemRow__row__preview__photo"
                         src={imgURL} onClick={() => {
                        if (!noImg.current)
                            setImgPreview(true)
                ...
                <div className="AudioItemRow__row__fimbo_name">
                    {fimboName(audio.fimbo_id)}
                </div>

                <div className="AudioItemRow__row__fimbo_size">
                    {audio.fimbo_size}
                </div>

                <div className="AudioItemRow__row__path">
                    {audio.path}
                </div>

                <div className="AudioItemRow__row__edit">
                    <Link to={'/audio/' + audio.id}>
                        <svg className="AudioItemRow__row__edit__pen" xmlns="http://www.w3.org/2000/svg"
