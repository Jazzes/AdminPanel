...
import LessonItemListen from "./LessonItemListen";
import {ILessonItem} from "./LessonItemForm";

interface ILessonItemNoteTable extends ILessonItem{
    l_beat: string
    l_tact: string
    l_square: string
    bpm: string
...
const LessonItemNoteTable: FC<ILessonItemNoteTable> = ({wasChanges, bpm, lesson, setL_square, l_tact, l_beat, l_square}) => {

    const [accents, setAccents] = useState<Array<boolean>>([])
    const [deleteString, setDeleteString] = useState<Array<boolean>>([])
    const [beatsVisible, setBeatsVisible] = useState<BeatEntity[]>([...lesson.lesson.beats])
    const beatsNoUpdate = useRef<BeatEntity[]>([...lesson.lesson.beats])
        ...
        <>
            ...
            <div className="LessonItemPage__form__notes__container">
                ...
                <div style={{maxWidth: `${Number(l_beat) * 20}px`, minWidth: `${Number(l_beat) * 20}px`}}
                     className="LessonItemPage__form__notes__table">
                    {beatsVisible.map((entity, index) =>
                        <div key={index + "ASC"}>
                            {(index + 1) % Number(l_beat) === 0 ?
                                <div key={index + "la"}>
                                    <div className="LessonItemPage__form__notes__table__hint" key={index + "laa"}
                                         style={{top: `${(index + 1) / Number(l_beat) * 62 - 31}px`}}>A
                                    </div>
                                    <div className="LessonItemPage__form__notes__table__hint" key={index + "lab"}
                                         style={{top: `${(index + 1) / Number(l_beat) * 62 - 11}px`}}>L
                                    </div>
                                    <div className="LessonItemPage__form__notes__table__hint" key={index + "lac"}
                                         style={{top: `${(index + 1) / Number(l_beat) * 62 + 9}px`}}>R
                                    </div>
                                    <div className="LessonItemPage__form__notes__table__numberString"
                                         key={index + "lag"}
                                         style={{top: `${(index + 1) / Number(l_beat) * 62 - 18}px`}}>
                                        <div className="LessonItemPage__form__notes__table__numberString__settings">
                                            <div
                                                className="LessonItemPage__form__notes__table__numberString__settings__item"
                                                onClick={() => {
                                            ...
                                            <div onClick={() => {
                                                addStringFromNote((index + 1))
                                                 ...
                                                 className="LessonItemPage__form__notes__table__numberString__settings__item_center">Строчка
                                                после
                                            </div>
                                            <div
                                                className="LessonItemPage__form__notes__table__numberString__settings__item"
                                                ...
                                                {!deleteString[(index + 1) / Number(l_beat) - 1] ?
                                                    ...
                                                    :
                                                    <div
                                                        className="LessonItemPage__form__notes__table__numberString__settings__item__delete">
                                                        <div
                                                            className="LessonItemPage__form__notes__table__numberString__settings__item__delete_yes"
                                                            onClick={() => {
                                                                deleteStringFromNote(index + 1)
                                                            }}>Да
                                                        </div>
                                                        <div
                                                            className="LessonItemPage__form__notes__table__numberString__settings__item__delete_no">Нет
                                                        </div>
                                                    </div>
                                                }
                            ...
                            <div key={index}
                                 ...
                                 className="LessonItemPage__form__notes__table__note">
                                <input style={{display: "none"}} onChange={() => {
                                }} name={"accent" + index}
                                ...
                                <input
                                    ...
                                    autoComplete={"off"}
                                    className={accents[index] ? "LessonItemPage__form__notes__table__input LessonItemPage__form__notes__table__input_orange" : "LessonItemPage__form__notes__table__input"}/>
                                <input
                                    style={(index + 1) % Number(l_tact) === 0 && (index + 1) % Number(l_beat) !== 0 ? {borderRight: "1px solid gray"} : (index + 1) % Number(l_beat) === 0 ? {borderRight: "0"} : {}}
                                    ...
                                    autoComplete={"off"}
                                    className="LessonItemPage__form__notes__table__input_center"/>
                                <input
                                    style={(index + 1) % Number(l_tact) === 0 && (index + 1) % Number(l_beat) !== 0 ? {borderRight: "1px solid gray"} : (index + 1) % Number(l_beat) === 0 ? {borderRight: "0"} : {}}
                                    ...
                                    autoComplete={"off"}
                                    className="LessonItemPage__form__notes__table__input"/>
                            </div>
                        </div>
                    )}
                </div>
...
export default LessonItemNoteTable;